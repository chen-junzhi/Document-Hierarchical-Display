        ...
        if(is_dir($dir)){ 
            ...
            foreach($child_dirs as $key => $child_dir){
                ...
                if($child_dir != '.' && $child_dir != '..'){
                    $suffix = 'images/'.strtolower(substr(strrchr($child_dir, '.'), 1)).'.jpg';
...
<head>
	...
	<script type="text/javascript" src="lib/zTree.js"></script>
...
<script type="text/javascript">
    ...
    function fCilck(event, treeId, treeNode){
        ...
        if( index !== -1 ){ 
            var suffix = treeNode.name.substring(index+1).toLowerCase() //后缀名;
            var name = treeNode.name;//点击的节点名
            var path = treeNode.path;//点击的节点路径
            if( suffix === 'mp4' || suffix === 'ogg' || suffix === 'mp3' ){
                $('#showInfo .fileName').html(treeNode.name);
                $('#showinfo .video').attr('src',path);
            }else{
                if( target.className === 'node_name' ){
                    target.parentNode.setAttribute('href',path);
